/**
 * dev.js — EXPERIMENTAL preview: serving scaler + US/Metric conversion done
 * client-side by parsing the free-text ingredient lines. Best-effort (no
 * structured data); lines it can't parse pass through unchanged. This page is a
 * sandbox to evaluate conversion before committing to a structured rebuild.
 */

import { supabase } from '../supabase.js';

// ── quantity parsing (shared idea with recipe.js) ──────────────────────────
const UF = {
...
// ── units (base: ml for volume, g for weight) ───────────────────────────────
const UNITS = {
...
// amount (in the given unit) → { amount, unit } in the target system
function convertUnit(amount, info, target) {
  const baseVal = amount * info.base; // ml or g
  if (target === 'metric') {
...
// Parse "<qty>[-<qty>] [unit] <rest>", scale by f, optionally convert to `system`.
function transform(line, f, system) {
  ...
  // no conversion: scale, keep whatever unit/word was there
...
function convertTemps(text, system) {
  ...
  return text.replace(/(\d{2,3})\s*°?\s*(?:F\b|degrees(?:\s*F)?)/gi, (_, f) => {
    return `${round5((+f - 32) * 5 / 9)}°C`;
  });
}

window.devPage = function devPage() {
  return {
    recipe: null,
    loading: true,
    error: null,
    ...
    async init() {
      ...
      q = id ? q.eq('id', id).maybeSingle() : q.order('updated_at', { ascending: false }).limit(1).maybeSingle();
      const { data, error } = await q;
      if (error) this.error = "Couldn't load a recipe.";
      else if (!data) this.error = 'No recipe found. Add one first, or pass ?id=.';
